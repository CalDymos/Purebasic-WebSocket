...
Module WebSocket 
  ...
  Declare _Connect_Thread(*This.sWebSocket)
  ...
  Procedure Connect(*This.sWebSocket)
    ...
    With *This
      ...
      msec = 0
      threadID = CreateThread(@_Connect_Thread(), *This)
  ...
  Procedure Send(*This.sWebSocket, message.s)
    ...
    *Param\msg\MessageType = Net::#WebSocketMessageType_Text
    ...
    With *This      
      SendTaskID = CreateThread(@_Send_Thread(),*Param)
      LockMutex(\objMutex)
  ...
  Procedure _Connect_Thread(*This.sWebSocket)
  ...
  Procedure _Send_Thread(*Param._sSendParam)
     With *Param\this
  ...
  Procedure _SendFrame(*This.sWebSocket, *payloadBuffer, *header.sMessageHeader)
    ...
    With *This
      payloadLength = MemorySize(*payloadBuffer)
            
      dbg("Payload data length to send: " + Str(payloadLength))
      
      ; The framebuffer, we fill it with the send data
      If payloadLength <= 125
        Headerlength = 6
      ElseIf payloadLength >= 126 And payloadLength <= 65535
        Headerlength = 8
      Else
      ...
      *FrameBuffer = AllocateMemory(Headerlength + payloadLength)
      ...
      If payloadLength <= 125                                             ; Length fits in first byte
        PokeA(*Framebuffer + pos, payloadLength + 128)    : pos + 1       ; + 128 for Masking
      ElseIf payloadLength >= 126 And payloadLength <= 65535                  ; We have to extend length to third byte
        PokeA(*Framebuffer + pos, 126 + 128)          : pos + 1       ; 126 for 2 extra length bytes and + 128 for Masking
        PokeA(*FrameBuffer + pos, (payloadLength >> 8))   : pos + 1       ; First Byte
        PokeA(*FrameBuffer + pos, payloadLength)          : pos + 1       ; Second Byte
      Else                                                            ; It's bigger than 65535, we also use 8 extra bytes
        ...
        PokeA(*Framebuffer + pos, 0)                  : pos + 1
        PokeA(*Framebuffer + pos, 0)                  : pos + 1
        PokeA(*Framebuffer + pos, payloadLength >> 24)    : pos + 1
        PokeA(*Framebuffer + pos, payloadLength >> 16)    : pos + 1
        PokeA(*Framebuffer + pos, payloadLength >> 8)     : pos + 1
        PokeA(*Framebuffer + pos, payloadLength)          : pos + 1       ; = 10 Byte
      EndIf
      ; Write Masking Bytes
      ...
      CopyMemory(*payloadBuffer, *FrameBuffer + pos, payloadLength)
            
      If SendNetworkData(\connectionID, *FrameBuffer, Headerlength + payloadLength) = Headerlength + payloadLength
        dbg("sent frame size in byte: " + Str(Headerlength + payloadLength))
        result = #True
      Else
        result = #False
      EndIf
  ...
  Procedure _Close(*This.sWebSocket, closeStatus.w, statusDescription.s)
    ...
    With *This
      ...
      ReDim Payload(header\PayloadLength - 1)
      PokeU(@Payload(), ((closeStatus & $FF00) >> 8) | ((closeStatus & $FF) << 8))
      PokeS(@Payload() + SizeOf(closeStatus), statusDescription, -1, #PB_UTF8|#PB_String_NoZero)      
      _SendFrame(*This.sWebSocket, Payload(), @header)
  ...
  Procedure _StartMonitor(*This.sWebSocket)
    Protected MonitorTaskId.i
    
    With *this
      dbg("Starting monitor.")
      MonitorTaskId = CreateThread(@_StartMonitor_Thread(), *This)
      LockMutex(\objMutex)
  ...
  Procedure _StartMonitor_Thread(*This.sWebSocket)
    Protected  lastState
  ...
  Procedure _DoReconnect(*This.sWebSocket)
    Protected ReconnectTaskID
    With *This
      dbg("Entered reconnect.")
      ReconnectTaskId = CreateThread(@_DoReconnect_Thread(), *This)
      LockMutex(\objMutex)
  ...
  Procedure _DoReconnect_Thread(*This.sWebSocket)
    Protected msec.l
  ...
  Procedure _StartListener(*This.sWebSocket)
    Protected ListenerTaskID
    With *This
      dbg("Starting listener.")
      ListenerTaskId = CreateThread(@_StartListener_Thread(), *This)
      LockMutex(\objMutex)
      \listenerTaskId = ListenerTaskID
      UnlockMutex(\objMutex)
    EndWith
  EndProcedure
  ...
  ;TODO:
  Procedure _StartListener_Thread(*This.sWebSocket)
    Protected message.s
    Protected Dim binary.a(0) : FreeArray(binary())
    Dim buffer.a(0) : FreeArray(buffer())
      ...
      If headerErrorCode <> 0
        ...
        ElseIf header\Opcode = #MessageOpcode_Ping 
          ...
          *res\Count = header\PayloadLength
          ...
          _SendFrame(*This, buffer(), header) ; and return Message
          FreeMemory(*FrameBuffer)
        ...
        Dim buffer(header\PayloadLength - 1)
          ;Dbg("ArraySize buffer: " + ArraySize(buffer()))
          CopyMemory(*FrameBuffer + BufOffset, @buffer(), header\PayloadLength)
          *res\Count = header\PayloadLength
        ...
        If header\Mask
          *res\MessageType = Net::#WebSocketMessageType_invalid ; all messages from the server to the client MUST NOT be masked !
          ...
          _ApplyMasking(header\Mask, @buffer())
  ...
  Procedure _StartSender(*This.sWebSocket)
    Protected SenderTaskID
    With *This
      dbg("Starting Sender.")
      SenderTaskId = CreateThread(@_StartSender_Thread(), *This)
      LockMutex(\objMutex)
  ...
  Procedure _StartSender_Thread(*This.sWebSocket)
